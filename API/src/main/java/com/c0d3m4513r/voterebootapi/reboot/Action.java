...
public abstract class Action implements Runnable{
    @NonNull
    public static Vector<Action> actions = new Vector<>();
    @NonNull
    ...
    protected Action(@NonNull RestartType restartType) {
    ...
    public Optional<Long> getTimer(Permission perm) {
        if (perm.hasPerm(restartType.perm.getReadPermission()))
    ...
    public boolean cancelTimer(Permission perm){
        if (perm.hasPerm(restartType.perm.getCancelPermission()) && task.isPresent()) return task.get().cancel();
        else return false;
    }
    ...
    public boolean reset(Permission perm){
        if (perm.hasPerm(restartType.perm.getStartPermission())){
            if(task.isPresent()){
                if(!task.get().cancel())
                    return false;
            }
    ...
    public boolean modifyTimer(Permission perm, long time, TimeUnit unit, Function<Long, LongUnaryOperator> operation){
        if (perm.hasPerm(restartType.perm.getModifyPermission())) {
